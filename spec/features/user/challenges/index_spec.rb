...
RSpec.describe 'User ' do
  before(:each) do
    ...
    @jans_tuesday_lunch = @janis.challenges.create!(time_limit: 20, basket_size: 7, meal_type: "lunch")
    ChallengeIngredient.create!(challenge_id: @jans_tuesday_lunch.id, ingredient_id: @quail_eggs.id)
    ChallengeIngredient.create!(challenge_id: @jans_tuesday_lunch.id, ingredient_id: @sprite.id)
    ChallengeIngredient.create!(challenge_id: @jans_tuesday_lunch.id, ingredient_id: @peanut_butter.id)
    ChallengeIngredient.create!(challenge_id: @jans_tuesday_lunch.id, ingredient_id: @avacado.id)
    ChallengeIngredient.create!(challenge_id: @jans_tuesday_lunch.id, ingredient_id: @lemon.id)
    ChallengeIngredient.create!(challenge_id: @jans_tuesday_lunch.id, ingredient_id: @chocolate.id)
    ChallengeIngredient.create!(challenge_id: @jans_tuesday_lunch.id, ingredient_id: @cinnamon.id)
    ...
    @jans_other_snack = @janis.challenges.create!(time_limit: 20, basket_size: 7, meal_type: "snack")
    ChallengeIngredient.create!(challenge_id: @jans_other_snack.id, ingredient_id: @chocolate.id)
    ChallengeIngredient.create!(challenge_id: @jans_other_snack.id, ingredient_id: @cinnamon.id)
    ChallengeIngredient.create!(challenge_id: @jans_other_snack.id, ingredient_id: @eggs.id)
    ChallengeIngredient.create!(challenge_id: @jans_other_snack.id, ingredient_id: @toast.id)
    ChallengeIngredient.create!(challenge_id: @jans_other_snack.id, ingredient_id: @blueberries.id)
    ChallengeIngredient.create!(challenge_id: @jans_other_snack.id, ingredient_id: @lemon.id)
    ChallengeIngredient.create!(challenge_id: @jans_other_snack.id, ingredient_id: @avacado.id)
  ...
  it 'can see all the games that have been played' do
    @jans_other_snack.game_complete
    @jans_other_snack.reload
    ...
    @jans_lunch.reload
    @jans_tuesday_lunch.game_complete
    @jans_tuesday_lunch.reload

    expect(@jans_second_breakfast.game_status).to eq("before")
    expect(@jans_snack.game_status).to eq("complete")
    expect(@jans_other_snack.game_status).to eq("complete")
    ...
    expect(@jans_tuesday_lunch.game_status).to eq("complete")
    ...
    within "#7ingredients" do
      within "#challenge-#{@jans_tuesday_lunch.id}" do
        expect(page).to have_content("Basket Size: #{@jans_tuesday_lunch.basket_size}")
        expect(page).to have_button(@jans_tuesday_lunch.id)
        expect(page).to have_content(@quail_eggs.name)
        expect(page).to have_content(@sprite.name)
        expect(page).to have_content(@lemon.name)
        expect(page).to have_content(@avacado.name)
      ...
      within "#challenge-#{@jans_other_snack.id}" do
        expect(page).to have_content("Basket Size: #{@jans_other_snack.basket_size}")
        expect(page).to have_button(@jans_other_snack.id)
        expect(page).to have_content(@chocolate.name)
        expect(page).to have_content(@cinnamon.name)
        expect(page).to have_content(@eggs.name)
        expect(page).to have_content(@toast.name)
